...
from DBInterface import updateDAG_DB,checkPreviousRunForDAG_DB
...
# setting branch to "BANK" for bank DAG
branch = "BANK"
...
if json_file_path is None or not json_file_path:
    ...
    if not os.path.exists(bank_config_json_file):
        logger.error("Bank configuration json file : {0} not found".format(bank_config_json_file))
        no_of_errors += 1
    else:
        with open(bank_config_json_file, "r") as fp:
            config = fp.read()
...
if no_of_errors == 0:
    ...
    updateBankDAG = PythonOperator(
        task_id='UPDATE_BANK_STATUS',
        python_callable=updateDAG_DB,
        op_kwargs={"branch_code": '{0}'.format(branch),
                   'tid': "UPDATE_BANK_STATUS",
                   "logging": logger,
                   "postgres": postgres_info,
                   "data_pipeline": data_pipeline,
    ...
    success >> updateBankDAG
    fail_notrun >>  moduleNode
    ...
    for moduletotable in modules_list:
        ...
        for table in input_table_list:
            ...
            airflowModule >> airflowTable >> updateBankDAG
